...
Open `workshop-app/Program.cs`. Build the program in small additions so you can see where each
...
`using GitHub.Copilot;` makes the SDK types available. The heading separates the workshop output
from the commands you run in the terminal.
...
`CopilotClient` manages the application-wide connection to the local Copilot CLI runtime.
`StartAsync` launches or connects to that runtime before any requests are sent.
...
`PingAsync` performs a lightweight health check, giving you an immediate confirmation that the app
can reach Copilot. Print the returned message so connection failures are obvious before you create
a session.
...
A `CopilotSession` owns one conversation and its context. With an empty `SessionConfig`, the
runtime uses your account's default model. `SendAndWaitAsync` sends the prompt and waits until the
session is idle.
...
The guard fails clearly if Copilot finishes without an assistant message. Otherwise, print the
message content so you can inspect the response in the terminal.
...
another choice. Now add explicit model selection for the next run.
