...
`workshop-app`. Every starter includes the application-owned Web Content Accessibility Guidelines
(WCAG) rule catalog and the scoped permission/snapshot-reader boundary introduced in Step 4. The
Copilot client, session, streaming flow, local-tool wiring, and MCP configuration are added during
the lessons.
...
| Go | [Go 1.24+](https://go.dev/dl/) | `cp -R start/go workshop-app && cd workshop-app && go build ./...` |
